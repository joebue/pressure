        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.pressure.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.pressures.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('sys') ? 'has-error' : '' }}">
                            <label class="required" for="sys">{{ trans('cruds.pressure.fields.sys') }}</label>
                            <input class="form-control" type="number" name="sys" id="sys" value="{{ old('sys', '0') }}" step="1" required>
                            @if($errors->has('sys'))
                        ...
                        <div class="form-group {{ $errors->has('dia') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="dia" id="dia" value="{{ old('dia', '') }}" step="1" required>
                            @if($errors->has('dia'))
                        ...
                        <div class="form-group {{ $errors->has('pulse') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="pulse" id="pulse" value="{{ old('pulse', '') }}" step="1" required>
                            @if($errors->has('pulse'))
